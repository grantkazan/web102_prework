...
const backsum = GAMES_JSON.reduce((acc, game) => {
    return acc + game.backers;
}, 0);


// set the inner HTML using a template literal and toLocaleString to get a number with commas
contributionsCard.innerHTML = backsum.toLocaleString('en-US');
console.log("total backers: ", backsum)
// grab the amount raised card, then use reduce() to find the total amount raised

const raisedCard = document.getElementById("total-raised");

// set inner HTML using template literal
const totraised = GAMES_JSON.reduce((acc, game) => {
...
console.log("total backers: ", totraised)
...
function filterUnfundedOnly() {
    ...
    console.log("unfunded games ", listUnfundedGames.length)
...
function filterFundedOnly() {
    ...
    console.log("funded games ", listFundedGames.length)
...
const displayStr = `A total of $${GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0).toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${listUnfundedGames.length === 1 ? `${listUnfundedGames.length} game remains` : `${listUnfundedGames.length} games remain`} unfunded. We need your help to fund these amazing games!`;
